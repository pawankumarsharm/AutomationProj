...
using SelectCartridgeChile.Logger;
using System;
using System.Threading;
using TechTalk.SpecFlow;

namespace SelectCartridgeChile.StepsDefination
{
    [Binding]
    public class ChileSelectSteps:Containers
    {
        public static ILog Logger = GenerateLogs.GetLogger(typeof(ChileSelectSteps));
        ...
        [Then(@"I select the  (.*) provided  and click on the next button")]
        public void ThenISelectTheProvidedAndClickOnTheNextButton(string _region)
        {
            string _countryRegion = _region.ToLower();
            if (_countryRegion.Equals("chile"))
            {
                Logger.Info("Selected region as chile.");
                BrowserConfigs.webDriver.FindElement(By.XPath(Containers._chilecountry)).Click();
                Thread.Sleep(4000);
            }
        ...
        public void ThenISelectTheFilteredProductsUsingTheCasOrTheContaminantsValue(string cas, string contaminants)
        {
        ...
        public void ThenIFilterTheContaminantsUsingTheFilterValuesAnd(string _gasQuestions, string _oilQuestion, string _particle, string _respiratorType, string _filterClass, string _cartidgeType, string _casNumber, string _txtMsg)
        {
            string _txtOil = "¿Hay presencia de aerosoles de aceites?";
           // string _txtGas = "Sus contaminantes son partículas. ¿También existen niveles no peligrosos de gases o vapores?";
            string _txtGas = "Sus contaminantes son gases o vapores. ¿También hay partículas presentes (por ejemplo, niebla de pintura en aerosol)?";
            ...
            if (_txtMsg != null)
            {
                ...
                bool _trigger = (_oilQuestion == null && _particle == null && _gasQuestions == null);
                if (_trigger)
                {
                    Logger.Info("No filters available for one or more of your contaminants");
                }
